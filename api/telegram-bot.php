...
$maximumBytes = da_config_int($config, 'telegram_request_max_bytes', 'LEAD_TELEGRAM_REQUEST_MAX_BYTES', 1048576, 1024, 4194304);
...
if ($contentLength !== false && $contentLength !== null && $contentLength > $maximumBytes) {
    da_respond(['ok' => false, 'message' => 'Обновление Telegram слишком большое.'], 413);
}

$rawBody = @file_get_contents('php://input', false, null, 0, $maximumBytes + 1);

if (is_string($rawBody) && strlen($rawBody) > $maximumBytes) {
    da_respond(['ok' => false, 'message' => 'Обновление Telegram слишком большое.'], 413);
}
...
if ($adminIds === []) {
    $rawAdminIds = trim((string) (getenv('TELEGRAM_ADMIN_IDS') ?: ''));
    $adminIds = array_values(array_filter(array_map('trim', explode(',', $rawAdminIds))));
}

if ($adminIds === []) {
    $replyError = replyTelegram($config, $chatId, 'Администраторы промокодов не настроены. Настройте TELEGRAM_ADMIN_IDS на сервере.');
